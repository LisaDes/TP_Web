...
class Grille {
  ...
  APasCookieVide(){
    ...
    for(let l = 1; l < this.nbLignes; l++){
      for(let c = 0; c < this.nbColonnes; c++){
        let cookieNow = this.tabCookies[l][c].isASupprimer();
        cookieVide += 1;
      }
    }
  ...
  chute(){
      for(let l = 0; l < this.nbLignes; l++){
        for(let c = 0; c < this.nbColonnes; c++){
          //console.log("chute");
          let cookieNow = this.tabCookies[l][c];
  
          if(l === 0){
            if (cookieNow.isASupprimer()){
              let type = Math.floor(Math.random() * this.nbCookiesDifferents);
              cookieNow.type = type;
              cookieNow.htmlImage.src = Cookie.urlsImagesNormales[type];
              console.log("CookieNow = type :" + cookieNow.type);
              
              cookieNow.annulerASupprimer();
            }
          }else{
            if(cookieNow.isASupprimer()){

            let cookieTombe = this.tabCookies[l-1][c];

            if(!cookieTombe.isASupprimer()){
              cookieNow.htmlImage.src = cookieTombe.htmlImage.src;
              cookieNow.type = cookieTombe.type;
              cookieNow.annulerASupprimer();
  
              cookieTombe.supprimer();
            }else{
              let type = Math.floor(Math.random() * this.nbCookiesDifferents);
              cookieNow = new Cookie(type, l, c);
            }
